...
<form class="input-group mb-3 mt-3 w-25 searchBar" method="post">
    ...
    <h2 style="margin-top:2vh;margin-bottom:2vh;">Solved activities</h2>
    <?php include "../functions.php"; showAdminActivity(2);?>
</body>
...
<?php
    ...
    if(isset($_POST["edit"])) {
        header("adminResolved.php");
        exit();
    }
    if(isset($_POST["save"])) {
        changeWorker($_POST["save"], $_POST['worker'],3);
        exit();
    }
    if(isset($_POST["error"])) {
        echo "<script>alert(\"You cannot modify the worker without clicking the edit button\")</script>";
        exit();
    }
    if(isset($_POST["sendDate"])) {
        changeDate($_POST["sendDate"], $_POST["date"],3);
        exit();
    }
